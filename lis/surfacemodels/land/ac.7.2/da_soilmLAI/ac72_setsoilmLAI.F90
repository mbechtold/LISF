...
!BOP
! !ROUTINE: ac72_setsoilmLAI
!  \label{ac72_setsoilmLAI}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine ac72_setsoilmLAI(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
! !DESCRIPTION:
!  
!  This routine assigns the soil moisture and LAI prognostic variables to ac72's
!  model space. 
! 
!EOP
...
! mn
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in ac72_setsoilmLAI")

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in ac72_setsoilmLAI")
  
  ! Layer 2
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in ac72_setsoilmLAI")

  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in ac72_setsoilmLAI")
  
  ! Layer 3
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in ac72_setsoilmLAI")

  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in ac72_setsoilmLAI")
...
end subroutine ac72_setsoilmLAI
