...
!BOP
! !ROUTINE: ac72_write_soilmLAI
! \label{ac72_write_soilmLAI}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac72_write_soilmLAI(ftn,n, LSM_State)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC72_struc(n)%ac72(t)%smc(1)
  enddo  
  call LIS_writevar_restart(ftn,n,1,tmp)

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC72_struc(n)%ac72(t)%smc(2)
  enddo
  call LIS_writevar_restart(ftn,n,1,tmp)

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC72_struc(n)%ac72(t)%smc(3)
  enddo
  call LIS_writevar_restart(ftn,n,1,tmp)

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC72_struc(n)%ac72(t)%smc(4)
  enddo
  call LIS_writevar_restart(ftn,n,1,tmp)
  deallocate(tmp)

end subroutine ac72_write_soilmLAI
