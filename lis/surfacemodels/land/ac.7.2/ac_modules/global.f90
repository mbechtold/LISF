...
subroutine CalculateSoftSWCReset(CompartAdj)
    ...
    integer(int32) :: compi
    integer(int32) :: layer, compi_above, compi_below, NrC
    real(sp) :: TAWcompi, ThetaThreshold, ThetaThresholdDonor
    real(sp) :: deficit, move, available, pZexp
    real(sp) :: receiver_theta, donor_theta

    NrC  = GetNrCompartments()
    pZexp = GetCrop_pdef() + (1.0_sp - GetCrop_pdef())/2.0_sp

    do compi = 4, (NrC-1)
        ! receiver (middle)
        layer = GetCompartment_Layer(compi)
        TAWcompi = GetSoilLayer_FC(layer)/100.0_sp - GetSoilLayer_WP(layer)/100.0_sp
        ThetaThreshold = GetSoilLayer_FC(layer)/100.0_sp - pZexp * TAWcompi

        receiver_theta = GetCompartment_theta(compi)
        deficit = ThetaThreshold - receiver_theta  ! positive if water is lacking

        if (deficit > epsilon(0._sp)) then
            ! -------- look above for water: from compi-1 up to 3 (never 1 or 2) --------
            compi_above = compi - 1
            do
                if ((deficit <= epsilon(0._sp)) .or. (compi_above == 2)) exit

                layer = GetCompartment_Layer(compi_above)
                TAWcompi = GetSoilLayer_FC(layer)/100.0_sp - GetSoilLayer_WP(layer)/100.0_sp
                ThetaThresholdDonor = GetSoilLayer_FC(layer)/100.0_sp - pZexp * TAWcompi

                donor_theta = GetCompartment_theta(compi_above)
                available = donor_theta - ThetaThresholdDonor   ! only water above donor's threshold
                if (available > epsilon(0._sp)) then
                    move = min(deficit, available)
                    call SetCompartment_theta(compi_above, donor_theta - move)
                    call SetCompartment_theta(compi, GetCompartment_theta(compi) + move)
                    deficit = deficit - move
                end if

                compi_above = compi_above - 1
            end do

            ! -------- if still dry, look below for water: compi+1 .. NrC --------
            if (deficit > epsilon(0._sp)) then
                compi_below = compi + 1
                do
                    if ((deficit <= epsilon(0._sp)) .or. (compi_below == NrC + 1)) exit

                    layer = GetCompartment_Layer(compi_below)
                    TAWcompi = GetSoilLayer_FC(layer)/100.0_sp - GetSoilLayer_WP(layer)/100.0_sp
                    ThetaThresholdDonor = GetSoilLayer_FC(layer)/100.0_sp - pZexp * TAWcompi

                    donor_theta = GetCompartment_theta(compi_below)
                    available = donor_theta - ThetaThresholdDonor
                    if (available > epsilon(0._sp)) then
                        move = min(deficit, available)
                        call SetCompartment_theta(compi_below, donor_theta - move)
                        call SetCompartment_theta(compi, GetCompartment_theta(compi) + move)
                        deficit = deficit - move
                    end if
