...
!nothing to be done
  ...
  call LIS_verify(status, 'attributeget error in ac72_transform_LAI')
  call ESMF_StateGet(OBS_State,"Observation01",obs_lai_field,&
       rc=status)
  call LIS_verify(status,'stateget error in ac72_transform_LAI')
  call ESMF_FieldGet(obs_lai_field,localDE=0,farrayPtr=laiobs,rc=status)
  call LIS_verify(status,'fieldget error in ac72_transform_LAI')
