...
!BOP
! !ROUTINE: ac72_getSig0VVpred
! \label{ac72_getSig0VVpred}
!
! !REVISION HISTORY:
...
subroutine ac72_getSig0VVpred(n,k,obs_pred)
...
!LIS_histDAtaMod
  call LIS_verify(status, &
       "Error in StateGet in ac72_getSig0VVPred for WCM_Sig0VV")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=s0VV, rc=status)!variable
!s0VV previously defined
  call LIS_verify(status, &
       'Error in FieldGet in ac72_getSig0VVPred for WCM_Sig0VV')
...
end subroutine ac72_getSig0VVpred
