...
!BOP
! !ROUTINE: ac72_qc_Sig0VVobs
! \label{ac72_qc_Sig0VVobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac72_qc_Sig0VVobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!-----this part is derived from ./lis/dataassim/obs/s1_sigma/read_S1_sigma.F90
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in ac72_qc_Sig0VVobs sigmaField")

  call ESMF_FieldGet(sigmaField,localDE=0,farrayPtr=obsl,rc=status)
  call LIS_verify(status,& 
       "ESMF_FieldGet failed in ac72_qc_Sigobs obsl")
...
end subroutine ac72_qc_Sig0VVobs
