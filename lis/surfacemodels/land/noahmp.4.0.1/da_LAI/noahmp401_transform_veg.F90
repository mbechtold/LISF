...
!BOP
! !ROUTINE: noahmp401_transform_veg
! \label{noahmp401_transform_veg}
!
! !REVISION HISTORY:
!  22 Dec 2017: Sujay Kumar, Initial specification 
!
! !INTERFACE:
subroutine noahmp401_transform_veg(n,OBS_State)
...
!nothing to be done
  ...
  call LIS_verify(status, 'attributeget error in noahmp401_transform_LAI')
  call ESMF_StateGet(OBS_State,"Observation01",obs_lai_field,&
       rc=status)
  call LIS_verify(status,'stateget error in noahmp401_transform_LAI')
  call ESMF_FieldGet(obs_lai_field,localDE=0,farrayPtr=laiobs,rc=status)
  call LIS_verify(status,'fieldget error in noahmp401_transform_LAI')
  ...
  enddo
#endif
end subroutine noahmp401_transform_veg
