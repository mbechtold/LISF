...
!BOP
! !ROUTINE: noahmp50_qctws
! \label{noahmp50_qctws}
!
! !REVISION HISTORY:
!  May 2023: Cenlin He; modified for refactored NoahMP v5 and later
!
! !INTERFACE:
subroutine noahmp50_qctws(n, LSM_State)
...
!------- 
  ...
  call LIS_verify(status,&
       "ESMF_StateGet for Soil Moisture Layer 1 failed in noahmp50_qctws")

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 1 failed in noahmp50_qctws")
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Max Value failed in noahmp50_qctws")

  call ESMF_AttributeGet(gwField,"Min Value",gwsmin,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Min Value failed in noahmp50_qctws")
