...
!BOP
! !ROUTINE: NoahMP50_setsoilm
!  \label{NoahMP50_setsoilm}
!
! !REVISION HISTORY:
...
! mn
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in NoahMP50_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in NoahMP50_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in NoahMP50_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 4 failed in NoahMP50_setsoilm")

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in NoahMP50_setsoilm")
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in NoahMP50_setsoilm")
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in NoahMP50_setsoilm")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     !MN: delta = X(+) - X(-)
     !NOTE: "noahmp50_updatesoilm.F90" updates the soilm_(t)     
     delta1 = soilm1(t)-NoahMP50_struc(n)%noahmp50(t)%smc(1)
     delta2 = soilm2(t)-NoahMP50_struc(n)%noahmp50(t)%smc(2)
	  ...
	  if(update_flag_tile(t)) then
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           if(update_flag_tile(t)) then
              ...
              NoahMP50_struc(n)%noahmp50(t)%sh2o(1) = NoahMP50_struc(n)%noahmp50(t)%sh2o(1)+&
                   delta1
...
!-----------------------------------------------------------------------------------------
              ...
              NoahMP50_struc(n)%noahmp50(t)%sh2o(1) = smc_tmp 
              NoahMP50_struc(n)%noahmp50(t)%smc(1) = smc_tmp
                          
              smc_tmp = (MaxEnsSM2 - MinEnsSM2)/2 + MinEnsSM2            
              NoahMP50_struc(n)%noahmp50(t)%sh2o(2) = smc_tmp
              NoahMP50_struc(n)%noahmp50(t)%smc(2) = smc_tmp
               
              smc_tmp = (MaxEnsSM3 - MinEnsSM3)/2 + MinEnsSM3
              NoahMP50_struc(n)%noahmp50(t)%sh2o(3) = smc_tmp
              NoahMP50_struc(n)%noahmp50(t)%smc(3) = smc_tmp
              
              smc_tmp = (MaxEnsSM4 - MinEnsSM4)/2 + MinEnsSM4
              NoahMP50_struc(n)%noahmp50(t)%sh2o(4) = smc_tmp
              NoahMP50_struc(n)%noahmp50(t)%smc(4) = smc_tmp
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(m.ne.LIS_rc%nensem(n)) then 
                       delta(j) = delta(j) + &
                            ...
                            NoahMP50_struc(n)%noahmp50(t_unpert)%sh2o(j))
                    endif
              ...
              do j=1,4
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(tmpval.le.MIN_THRESHOLD) then 
                       NoahMP50_struc(n)%noahmp50(t)%sh2o(j) = &
                            max(NoahMP50_struc(n)%noahmp50(t_unpert)%sh2o(j),&
                            MIN_THRESHOLD)
                       NoahMP50_struc(n)%noahmp50(t)%smc(j) = &
                            max(NoahMP50_struc(n)%noahmp50(t_unpert)%smc(j),&
                            MIN_THRESHOLD)
                       ens_flag(m) = .false. 
                    elseif(tmpval.ge.sm_threshold) then
                       NoahMP50_struc(n)%noahmp50(t)%sh2o(j) = &
                            min(NoahMP50_struc(n)%noahmp50(t_unpert)%sh2o(j),&
                            sm_threshold)
                       NoahMP50_struc(n)%noahmp50(t)%smc(j) = &
                            min(NoahMP50_struc(n)%noahmp50(t_unpert)%smc(j),&
                            sm_threshold)
                       ens_flag(m) = .false. 
                    endif
                 enddo
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(ens_flag(m)) then 
                       ...
                       if(.not.(tmpval.le.0.0 .or.&
                          ...
                          NoahMP50_struc(n)%noahmp50(t)%smc(j) = &
                               NoahMP50_struc(n)%noahmp50(t)%smc(j) - delta(j)
                          NoahMP50_struc(n)%noahmp50(t)%sh2o(j) = &
                               NoahMP50_struc(n)%noahmp50(t)%sh2o(j) - delta(j)
...
!                    print*, i, m
!                    print*, 'smc',t, NoahMP50_struc(n)%noahmp50(t)%smc(:)
!                    print*, 'sh2o ',t,NoahMP50_struc(n)%noahmp50(t)%sh2o(:)
!                    print*, 'max ',t,MAX_THRESHOLD !NoahMP50_struc(n)%noahmp50(t)%smcmax
                    enddo
!                  call LIS_endrun()
                 enddo
              endif          
