...
! MN: check for frozen soil
        ...
        elseif(t1_obs(t).le.LIS_CONST_TKFRZ) then ! Var name Noah36 --> t1
           smobs(t) = LIS_rc%udef
        elseif((vegt_obs(t).le.4).and.(NOAHMP50_struc(n)%forestDA_opt.eq..false.)) then !forest types ! Var name Noah36 --> vegt
           smobs(t) = LIS_rc%udef
 ! MN: check for snow  
 ...
 ! MN: check for green vegetation fraction NOTE: threshold incerased from 0.5 to 0.7 
        elseif((shdfac_obs(t).gt.0.9).and.(NOAHMP50_struc(n)%forestDA_opt.eq..false.)) then  
...
!too close to the tails, could be due to scaling, so reject. 
        elseif(NOAHMP50_struc(n)%QC_opt.eq..true.) then
