...
!BOP
! 
! !ROUTINE: enkf_analysis
! \label{enkf_analysis}
! 
! !INTERFACE:  
  subroutine enkf_analysis( gid, &
       N_state, N_obs, N_ens, &
...
!          if(Observations(i)%pert_type.eq.0) then 
             rhs(i,n_e) = Observations(i)%value + Obs_err(i,n_e) - Obs_pred(i,n_e)
!          elseif(Observations(i)%pert_type.eq.1) then 
!             rhs(i,n_e) = Observations(i)%value * (1+Obs_err(i,n_e)) - Obs_pred(i,n_e)
!          endif
       end do
    enddo
...
! \label{hadamard_for_repr_matrix_enkf}
! 
! !INTERFACE:  
