...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_S1_sigmaVHSMLAI_037_D
! \label{read_S1_sigmaVHSMLAI_037_D}
!
! !REVISION HISTORY:
...
!
! !INTERFACE: 
subroutine read_S1_sigmaVHSMLAI_037_D(n,k,OBS_State,OBS_Pert_State) 
! !USES: 
  use ESMF
  use LIS_mpiMod
  use LIS_coreMod
  ...
  use LIS_pluginIndices, only : LIS_S1_sigmaVHSMLAI_037_D_obsId
  use S1_sigmaVHSMLAI_037_D_Mod, only : S1_sigma_struc
...
!-------------------------------------------------------------------------
  ...
  if(alarmCheck.or.S1_sigma_struc(n)%startMode) then 
     ...
     call S1_sigmaVHSMLAI_037_D_filename(S1_filename,obsdir,&
     ...
     if(file_exists) then 
        ...
        call read_S1_sigmaVHSMLAI_037_D_data(n,k, S1_filename, S1_sigma_struc(n)%sigma)
...
!-------------------------------------------------------------------------     
  ...
  call lsmdaqcobsstate(trim(LIS_rc%lsm)//"+"&
       //trim(LIS_S1_sigmaVHSMLAI_037_D_obsId)//char(0), & 
       n, k,OBS_state)
  ...
  if(data_upd) then 
     ...
     call ESMF_StateGet(OBS_Pert_State,"Observation01",pertfield,&
          rc=status)
     call LIS_verify(status, 'ESMF_StateGet for Observation01 for OBS_Pert_State failed in read_S1_sigmaVHSMLAI_037_D')
...
end subroutine read_S1_sigmaVHSMLAI_037_D



!BOP
!
! !ROUTINE: read_S1_sigmaVHSMLAI_037_D_data
! \label{read_S1_sigmaVHSMLAI_037_D_data}
!
! !INTERFACE:
subroutine read_S1_sigmaVHSMLAI_037_D_data(n, k, fname, sigma_ip)
!
! !USES:
...
end subroutine read_S1_sigmaVHSMLAI_037_D_data



!BOP
!
! !ROUTINE: S1_sigmaVHSMLAI_037_D_filename
! \label{S1_SWND_filename}
! 
! !INTERFACE: 
subroutine S1_sigmaVHSMLAI_037_D_filename(filename, ndir, yr, mo, da)
...
end subroutine S1_sigmaVHSMLAI_037_D_filename
