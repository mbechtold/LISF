...
!BOP
!
! !MODULE: S1_sigmaVVSMLAI_015_A_Mod
! 
! !DESCRIPTION: 
...
!-----------------------------------------------------------------------------
  public :: S1_sigmaVVSMLAI_015_A_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
...
!BOP
! 
! !ROUTINE: S1_sigmaVVSMLAI_015_A_setup
! \label{S1_sigmaVVSMLAI_015_A_setup}
! 
! !INTERFACE: 
  subroutine S1_sigmaVVSMLAI_015_A_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
...
!-------------------------------------------------------------
    ...
    call S1_sigmaVVSMLAI_015_A_filename(S1_filename,S1sigmaobsdir,2017,8,11)
    ios = nf90_open(path=trim(S1_filename),mode=NF90_NOWRITE,ncid=ncid)
    if(ios.ne.0) then 
      call S1_sigmaVVSMLAI_015_A_filename(S1_filename,S1sigmaobsdir,2017,8,12)
      ios = nf90_open(path=trim(S1_filename),mode=NF90_NOWRITE,ncid=ncid)
    endif
    if(ios.ne.0) then 
      call S1_sigmaVVSMLAI_015_A_filename(S1_filename,S1sigmaobsdir,2017,8,13)
      ios = nf90_open(path=trim(S1_filename),mode=NF90_NOWRITE,ncid=ncid)
    endif
    if(ios.ne.0) then 
      call S1_sigmaVVSMLAI_015_A_filename(S1_filename,S1sigmaobsdir,2017,8,14)
      ios = nf90_open(path=trim(S1_filename),mode=NF90_NOWRITE,ncid=ncid)
    endif
    if(ios.ne.0) then 
      call S1_sigmaVVSMLAI_015_A_filename(S1_filename,S1sigmaobsdir,2017,8,15)
      ios = nf90_open(path=trim(S1_filename),mode=NF90_NOWRITE,ncid=ncid)
    endif
    if(ios.ne.0) then 
      call S1_sigmaVVSMLAI_015_A_filename(S1_filename,S1sigmaobsdir,2017,8,16)
      ios = nf90_open(path=trim(S1_filename),mode=NF90_NOWRITE,ncid=ncid)
    endif
  ...
  end subroutine S1_sigmaVVSMLAI_015_A_setup
  
end module S1_sigmaVVSMLAI_015_A_Mod
