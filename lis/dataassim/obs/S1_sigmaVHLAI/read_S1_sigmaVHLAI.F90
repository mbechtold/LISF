...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_S1_sigmaVHLAI
! \label{read_S1_sigmaVHLAI}
!
! !REVISION HISTORY:
...
!
! !INTERFACE: 
subroutine read_S1_sigmaVHLAI(n,k,OBS_State,OBS_Pert_State) 
! !USES: 
  use ESMF
  use LIS_mpiMod
  use LIS_coreMod
  ...
  use LIS_pluginIndices, only : LIS_S1_sigmaVHLAI_obsId
  use S1_sigmaVHLAI_Mod, only : S1_sigma_struc
...
!-------------------------------------------------------------------------
  ...
  if(alarmCheck.or.S1_sigma_struc(n)%startMode) then 
     ...
     call S1_sigmaVHLAI_filename(S1_filename,obsdir,&
     ...
     if(file_exists) then 
        ...
        call read_S1_sigmaVHLAI_data(n,k, S1_filename, S1_sigma_struc(n)%sigma)
...
!-------------------------------------------------------------------------     
  ...
  call lsmdaqcobsstate(trim(LIS_rc%lsm)//"+"&
       //trim(LIS_S1_sigmaVHLAI_obsId)//char(0), & 
       n, k,OBS_state)
...
end subroutine read_S1_sigmaVHLAI



!BOP
!
! !ROUTINE: read_S1_sigmaVHLAI_data
! \label{read_S1_sigmaVHLAI_data}
!
! !INTERFACE:
subroutine read_S1_sigmaVHLAI_data(n, k, fname, sigma_ip)
!
! !USES:
...
end subroutine read_S1_sigmaVHLAI_data



!BOP
!
! !ROUTINE: S1_sigmaVHLAI_filename
! \label{S1_SWND_filename}
! 
! !INTERFACE: 
subroutine S1_sigmaVHLAI_filename(filename, ndir, yr, mo, da)
...
end subroutine S1_sigmaVHLAI_filename
