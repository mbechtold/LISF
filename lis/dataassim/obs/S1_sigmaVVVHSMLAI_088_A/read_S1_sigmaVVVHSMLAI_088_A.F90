...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_S1_sigmaVVVHSMLAI_088_A
! \label{read_S1_sigmaVVVHSMLAI_088_A}
!
! !REVISION HISTORY:
...
!
! !INTERFACE: 
subroutine read_S1_sigmaVVVHSMLAI_088_A(n,k,OBS_State,OBS_Pert_State) 
! !USES: 
  use ESMF
  use LIS_mpiMod
  use LIS_coreMod
  ...
  use LIS_pluginIndices, only : LIS_S1_sigmaVVVHSMLAI_088_A_obsId
  use S1_sigmaVVVHSMLAI_088_A_Mod, only : S1_sigma_struc
...
!  the local time of the grid point is 6PM (Ascending). 
! 
!  The arguments are: 
!  \begin{description}
...
!-------------------------------------------------------------------------
  ...
  if(alarmCheck.or.S1_sigma_struc(n)%startMode) then 
     ...
     call S1_sigmaVVVHSMLAI_088_A_filename(S1_filename,obsdir,&
     ...
     if(file_exists) then 
        ...
        call read_S1_sigmaVVVHSMLAI_088_A_data(n,k, S1_filename, S1_sigma_struc(n)%s_vv, &
             S1_sigma_struc(n)%s_vh)


!-------------------------------------------------------------------------
! Store the GMT corresponding to 6PM localtime at each grid point
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------     
  ...
  call lsmdaqcobsstate(trim(LIS_rc%lsm)//"+"&
       //trim(LIS_S1_sigmaVVVHSMLAI_088_A_obsId)//char(0), & 
       n, k,OBS_state)
...
end subroutine read_S1_sigmaVVVHSMLAI_088_A



!BOP
!
! !ROUTINE: read_S1_sigmaVVVHSMLAI_088_A_data
! \label{read_S1_sigmaVVVHSMLAI_088_A_data}
!
! !INTERFACE:
subroutine read_S1_sigmaVVVHSMLAI_088_A_data(n, k, fname, svv_ip, svh_ip)
!
! !USES:
...
end subroutine read_S1_sigmaVVVHSMLAI_088_A_data



!BOP
!
! !ROUTINE: S1_sigmaVVVHSMLAI_088_A_filename
! \label{S1_SWND_filename}
! 
! !INTERFACE: 
subroutine S1_sigmaVVVHSMLAI_088_A_filename(filename, ndir, yr, mo, da)
...
end subroutine S1_sigmaVVVHSMLAI_088_A_filename
