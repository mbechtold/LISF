...
!BOP
! 
! !ROUTINE: write_ML_SNWDobs
! \label{write_ML_SNWDobs}
! 
! !REVISION HISTORY: 
! 30 Aug 2019: Hans Lievens; Initial Specification
! 
! !INTERFACE: 
subroutine write_ML_SNWDobs(n, k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!  real                     :: testdata(LIS_rc%obs_lnc(k), LIS_rc%obs_lnr(k))
  ...
  if(data_update) then 
     ...
     if(LIS_masterproc) then 
        ...
        call ML_SNWD_obsname(n,k,obsname)        
...
end subroutine write_ML_SNWDobs

!BOP
! !ROUTINE: ML_SNWD_obsname
! \label{ML_SNWD_obsname}
! 
! !INTERFACE: 
subroutine ML_SNWD_obsname(n,k,obsname)
! !USES: 
...
end subroutine ML_SNWD_obsname
