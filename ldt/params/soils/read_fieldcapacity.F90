...
!
! !INTERFACE:
subroutine read_fieldcapacity(n,array)
...
! ___________________________________________________________________
  ...
  inquire(file=trim(LDT_rc%fieldcapacityfile(n)), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) "fieldcapacity map ",trim(LDT_rc%fieldcapacityfile(n))," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif
  write(unit=LDT_logunit,fmt=*) "[INFO] Reading fieleapcity file: ",&
                                trim(LDT_rc%fieldcapacityfile(n))
!- Open file:
   ftn = LDT_getNextUnitNumber()
   open( ftn,file=LDT_rc%fieldcapacityfile(n),form='unformatted',status='old',&
...
! ---
   ...
   write(unit=LDT_logunit,fmt=*) "[INFO] Done reading psisat soil texture file."

 end subroutine read_fieldcapacity
