...
! ___________________________________________________________________
  ...
  inquire(file=trim(LDT_rc%bexpfile(n)), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) "bexp map ",trim(LDT_rc%psisatfile(n))," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif
  write(unit=LDT_logunit,fmt=*) "[INFO] Reading pisiat file: ",&
                                trim(LDT_rc%bexpfile(n))
!- Open file:
   ftn = LDT_getNextUnitNumber()
   open( ftn,file=LDT_rc%bexpfile(n),form='unformatted',status='old',&
