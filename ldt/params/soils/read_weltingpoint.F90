...
!
! !INTERFACE:
subroutine read_weltingpoint(n,array)
...
! ___________________________________________________________________
  ...
  inquire(file=trim(LDT_rc%weltingpointfile(n)), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) "welting point map ",trim(LDT_rc%weltingpointfile(n))," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif
  write(unit=LDT_logunit,fmt=*) "[INFO] Reading weltingpoint file: ",&
                                trim(LDT_rc%weltingpointfile(n))
!- Open file:
   ftn = LDT_getNextUnitNumber()
   open( ftn,file=LDT_rc%weltingpointfile(n),form='unformatted',status='old',&
...
! ---
 ...
 end subroutine read_weltingpoint
