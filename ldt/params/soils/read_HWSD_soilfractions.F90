  ...
  !- Single layer transformation:
     case( "none", "average", "neighbor", "bilinear", "budget-bilinear" )
      ...
      ! Make sure and negative values are set to universal undefined value:
        do r=1,LDT_rc%lnr(n)
           do c=1,LDT_rc%lnc(n)     
           !adapted by MMI-KUL to avoid problems in clm2 (due to -9999 values in
           !soil mapswhihc are no gin landcover mask
              if( sandave(c,r,1) < 0. ) sandave(c,r,1) =1   !LDT_rc%udef
    
              if( clayave(c,r,1) < 0. ) clayave(c,r,1) =0    !LDT_rc%udef
              if( siltave(c,r,1) < 0. ) siltave(c,r,1) =0    ! LDT_rc%udef
           enddo
        enddo
        soilsfgrd(:,:,1) = 1.0
